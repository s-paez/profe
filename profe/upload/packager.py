...
class ExoFOPPackager:
        ...
        for obj_folder in sorted(self.data_dir.iterdir()):
            ...
            for date_folder in sorted(exofop_dir.iterdir()):
                ...
                if status in ("prepared", "uploaded"):
                    logger.info(
                        f"Target {target_name} on {local_date} is already {status}. Skipping."
                    )
                    continue
    ...
    def _process_date(
        self, target_name: str, local_date: str, date_folder: Path
    ) -> None:
        """Process a specific date folder for a target."""
        logger.info(
            f"Found pending upload for {target_name} on {local_date} at {date_folder}"
        )

        print(f"\n[{target_name} | {local_date}]")
        data_tag = input(
            f"Ingresa el Data Tag para {target_name} de la fecha {local_date}: "
        ).strip()

        if not data_tag:
            logger.warning(
                f"No Data Tag provided. Skipping {target_name} on {local_date}."
            )
            return
        ...
        if not collected_files:
            logger.warning(
                f"No files found to upload for {target_name} on {local_date}."
            )
            return
